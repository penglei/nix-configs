{
  ...
  outputs =
    ...
    let
      ...
      overlay-outputs = {
        overlays.default = lib.lists.foldr (f: acc: (final: prev: acc // (f final prev))) { } overlays;
      };
      ...
      homeConfigurations =
        let
          ...
          parse =
            s:
            let
              info = nixpkgs.lib.strings.splitString "." s;
            in
            {
            ...
            home-manager.lib.homeManagerConfiguration {
              ...
              ++ (
                if isDarwin then
                  ...
                  profiles.hm.linux.modules ++ [ { home.homeDirectory = "/home/${username}"; } ]
              );
            };

          hms = nixpkgs.lib.listToAttrs (
            map (target: {
              name = target;
          ...
          machines = builtins.mapAttrs (
            ...
            let
              config =
                (
                  options
                  // (lib.optionalAttrs (!(options ? hostname)) {
                    hostname = name;
                  })
